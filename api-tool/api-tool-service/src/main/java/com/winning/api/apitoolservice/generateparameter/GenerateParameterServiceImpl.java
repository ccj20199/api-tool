    ...
    public Map<String, Map<String, Object>> generateParameterDto(GenerateParameterInputVO inputVO) {
        ...
        try {
            ...
            s = CaseUtils.toCamelCase(s2, true, new char[]{'_'})+
                    ...
                    CaseUtils.toCamelCase(s1, true, new char[]{'_'})+"DTO";
        } catch (Exception e) {
            throw new BusinessException("url地址不符合规范：【"+apiUrl+"】");
        }
        log.info("api参数======"+JSON.toJSONString(apiParameter));
        Map<String, Map<String, Object>> mapData = apiParamInfoUtil.getMapData(apiParameter, s);
        log.info("map======"+JSON.toJSONString(mapData));
        return  mapData;
    }
}
